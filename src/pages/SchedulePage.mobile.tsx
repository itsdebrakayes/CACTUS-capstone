...
export default function SchedulePage() {
  ...
  return (
    <AppLayout activeTab="schedule">
      {/* Header */}
      <div className="glass-panel rounded-none border-l-0 border-r-0 border-t-0 px-4 pt-12 pb-3 sticky top-0 z-10">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-2xl font-black text-[hsl(var(--cactus-ink))] tracking-tight leading-none">Schedule</h1>
            <p className="text-[11px] text-slate-500 mt-1 uppercase tracking-widest">This Week</p>
          </div>
          <button className="w-10 h-10 rounded-2xl cactus-honey-grad flex items-center justify-center shadow-md shadow-amber-500/30">
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-white/70 border border-white/60 rounded-2xl text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-300/60 transition-all"
          />
        </div>

        {/* Tabs */}
        <div className="flex gap-1 bg-white/60 border border-white/60 rounded-2xl p-1">
          {[
            ...
            <button
              ...
              className={cn(
                ...
                activeTab === tab.id
                  ? "bg-[hsl(var(--cactus-ink))] text-white shadow-md"
      ...
      <div className="bg-white/50 backdrop-blur border-b border-white/60 px-4 py-3">
            ...
            return (
              ...
              >
                <span className={cn(
                  ...
                  isSelected ? "text-[hsl(var(--cactus-honey-2))]" : "text-slate-400"
                )}>
                  {label}
                </span>
                <div className={cn(
                  "w-9 h-9 rounded-2xl flex items-center justify-center text-sm font-black transition-all",
                  isSelected
                    ? "cactus-honey-grad text-white shadow-md shadow-amber-500/30"
                    : isToday
                    ? "bg-[hsl(var(--cactus-honey)/0.18)] text-[hsl(var(--cactus-honey-2))]"
                    : "text-slate-700"
                )}>
                  {date}
                </div>
      ...
      <div className="px-4 py-4">
        {activeTab === "events" ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-slate-500">Campus events coming soon</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm font-semibold text-slate-700 mb-1">No classes today</p>
                ...
                <div key={cls.id}>
                  {showTime && (
                    <div className="flex items-center gap-3 py-2">
                      <span className="text-xs text-slate-500 font-semibold w-20 shrink-0 tabular-nums">
