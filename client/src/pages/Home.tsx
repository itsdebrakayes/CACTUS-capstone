...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      {/* Header */}
      <header className="px-6 py-4 flex items-center justify-between border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-primary-foreground font-bold text-lg bg-primary">C</div>
          <div>
            <p className="font-bold text-sm leading-none text-foreground">CACTUS</p>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-16 text-center">
        <div className="max-w-xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium mb-6 border border-primary/20 bg-teal-light text-primary">
          ...
          <p className="text-base text-muted-foreground mb-8 leading-relaxed font-serif">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button
              size="lg"
              className="text-sm font-semibold bg-primary hover:bg-primary/90 text-primary-foreground"
              onClick={() => window.location.href = getLoginUrl()}>
              Get Started
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {FEATURES.map(({ icon: Icon, title, desc }) => (
              <div key={title}
                ...
                <div className="w-9 h-9 rounded-lg flex items-center justify-center mb-3 bg-teal-light">
                  <Icon className="w-5 h-5 text-primary" />
                </div>
                <p className="font-semibold text-sm mb-1 text-foreground">{title}</p>
                <p className="text-xs text-muted-foreground leading-relaxed font-serif">{desc}</p>
              </div>
            ))}
          </div>
        </div>
