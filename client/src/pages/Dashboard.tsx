...
export default function Dashboard() {
  ...
  const [events, setEvents] = useState<
    Array<{ event: RealtimeEvent; id: number }>
  >([]);
  const eventIdRef = useRef(0);

  // Data queries
  const { data: reports, refetch: refetchReports } =
    trpc.reports.getReports.useQuery({});
  ...
  const handleSSEEvent = useCallback(
    (event: RealtimeEvent) => {
      ...
      if (
        event.type === "reports.created" ||
        event.type === "reports.voted" ||
        event.type === "reports.expired"
      ) {
        refetchReports();
      }
      if (event.type === "checkins.failed") {
        toast.error("⚠️ Check-in FAILED — emergency contact notified!", {
          duration: 8000,
        });
      }
      if (
        event.type === "walking.match.updated" &&
        (event.data as any)?.status === "accepted"
      ) {
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 text-xs">
            <MapPin
              className={`w-3.5 h-3.5 ${locationGranted ? "" : "opacity-40"}`}
              style={locationGranted ? { color: "oklch(0.65 0.12 185)" } : {}}
            />
            <span
              className={`hidden sm:inline ${locationGranted ? "" : "opacity-40"}`}
              style={locationGranted ? { color: "oklch(0.65 0.12 185)" } : {}}
            >
          ...
          {user && (
            <div className="flex items-center gap-2 ml-1">
              ...
              <Button
                variant="ghost"
                size="sm"
                className="h-7 px-2 opacity-70 hover:opacity-100 hover:bg-white/10"
                style={{ color: "white" }}
                onClick={logout}
              >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 relative">
          <CactusMap
            ...
            onHazardClick={h =>
              toast.info(
                `${h.reportType}: Severity ${h.severity} — ${h.ttlMinutes}min TTL`
              )
            }
          />
          ...
          <div className="absolute top-4 left-4 flex gap-2 z-10">
            <div className="bg-white/90 backdrop-blur-sm rounded-lg px-2.5 py-1.5 text-xs font-medium shadow-sm border flex items-center gap-1.5">
              ...
              {hazardMarkers.length} hazard
              {hazardMarkers.length !== 1 ? "s" : ""}
            </div>
            <div className="bg-white/90 backdrop-blur-sm rounded-lg px-2.5 py-1.5 text-xs font-medium shadow-sm border flex items-center gap-1.5">
        ...
        <div
          className={`flex flex-col bg-background border-l transition-all duration-300 ${panelCollapsed ? "w-10" : "w-80"} shrink-0`}
        >
          <button
                  ...
                  return (
                    <button
                      ...
                      style={
                        activePanel === tab
                          ? {
                              borderColor: "oklch(0.55 0.12 185)",
                              color: "oklch(0.28 0.08 245)",
                            }
                          : {}
                      }
                    ...
                    >
                      ...
                      {tab === "feed" && events.length > 0 && (
                        <span
                          className="ml-0.5 text-white text-[9px] rounded-full px-1 min-w-[16px] text-center"
                          style={{ background: "oklch(0.55 0.12 185)" }}
                        >
                ...
                <div className="p-3">
                  ...
                  {activePanel === "actions" && (
                    <ActionPanels
                      ...
                      onCheckinCreated={id =>
                        toast.success(`Check-in #${id} started`)
                      }
                    />
                  )}
                  {activePanel === "feed" && (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div
                          className={`flex items-center gap-1 text-[10px] ${sseConnected ? "text-teal-600" : "text-muted-foreground"}`}
                        >
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${sseConnected ? "bg-teal-500 animate-pulse" : "bg-muted-foreground"}`}
                          />
                      ...
                      {events.length === 0 ? (
                        ...
                        events.map(({ event, id }) => (
                          <div
                            key={id}
                            className="event-entry flex gap-2 p-2 rounded-lg bg-muted/40 text-xs"
                          >
