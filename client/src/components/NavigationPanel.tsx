/**
 * NavigationPanel — Apple Maps-style navigation overlay for the CACTUS map.
 *
 * Two modes:
...
export const FACULTY_HUBS: FacultyHub[] = [
  ...
  {
    id: "fst-tasties",
    name: "Tasties Canteen (FST)",
    shortName: "Tasties",
    faculty: "FST",
    lat: 18.0042,
    lng: -76.7488,
    description: "Main canteen near FST block",
  },
  {
    id: "fst-eng-parking",
    name: "Engineering Parking Lot",
    shortName: "Eng. Parking",
    faculty: "FST",
    lat: 18.0038,
    lng: -76.7501,
    description: "Parking lot beside Engineering block",
  },
  {
    id: "fst-slt2",
    name: "SLT 2 Lecture Theatre",
    shortName: "SLT 2",
    faculty: "FST",
    lat: 18.0035,
    lng: -76.7497,
    description: "Science Lecture Theatre 2 — main campus hub",
  },
  {
    id: "fst-guild",
    name: "FST Guild Office",
    shortName: "FST Guild",
    faculty: "FST",
    lat: 18.0031,
    lng: -76.7493,
    description: "Faculty of Science & Technology Guild Office",
  },
  // Mona general hubs
  {
    id: "mona-main-gate",
    name: "Main Gate (Ring Road)",
    shortName: "Main Gate",
    faculty: "General",
    lat: 18.0028,
    lng: -76.7510,
    description: "Main entrance to UWI Mona campus",
  },
  {
    id: "mona-library",
    name: "Main Library",
    ...
    lat: 18.0040,
    lng: -76.7505,
    description: "UWI Mona Main Library",
  },
  {
    id: "mona-guild",
    ...
    lat: 18.0033,
    lng: -76.7485,
    description: "Guild of Students building",
  },
  {
    id: "mona-chapel",
    ...
    lat: 18.0045,
    lng: -76.7500,
    description: "UWI Mona University Chapel",
  },
];
...
/** Interpolate a position along a GeoJSON LineString at fraction t ∈ [0,1] */
...
export default function NavigationPanel({
  ...
  useEffect(() => {
    ...
    // Simulated speed: real duration but capped at 3x real-time for demo
    const simDuration = Math.min(routeDurationSec, 180); // max 3 min demo
    ...
    animFrameRef.current = setInterval(() => {
      setElapsedSec((prev) => {
        ...
        // Update simulated marker on map
        const map = mapRef.current?.getMap();
        if (map) {
          if (!simMarkerRef.current) {
            const el = document.createElement("div");
            el.className = "sim-walker-dot";
            el.style.cssText =
              "width:18px;height:18px;border-radius:50%;background:#00c853;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3);";
            const mapboxgl = (window as any).mapboxgl;
            if (mapboxgl) {
              simMarkerRef.current = new mapboxgl.Marker({ element: el })
                .setLngLat(pos)
                .addTo(map);
            }
          } else {
  ...
  // Cleanup sim marker on unmount / stop
  ...
  const progressPct =
    routeDurationSec > 0 ? Math.min((elapsedSec / Math.min(routeDurationSec, 180)) * 100, 100) : 0;
  const remainingSec = Math.max(0, Math.min(routeDurationSec, 180) - elapsedSec);
  const remainingMin = Math.ceil(remainingSec / 60);

  const filteredHubs =
    hubFilter === "All" ? FACULTY_HUBS : FACULTY_HUBS.filter((h) => h.faculty === hubFilter);

  // ── Collapsed search bar (Apple Maps style) ──────────────────────────────
  if (!expanded && navMode === "idle") {
    return (
      <div className="absolute top-4 left-4 right-4 z-20">
        <button
          onClick={() => setExpanded(true)}
          className="w-full bg-white/97 backdrop-blur-sm rounded-2xl shadow-lg px-4 py-3 flex items-center gap-3 text-left hover:shadow-xl transition-shadow"
        >
          <Search className="w-4 h-4 text-gray-400 shrink-0" />
          <span className="text-sm text-gray-400 flex-1">Where would you like to go?</span>
          <Navigation className="w-4 h-4 text-[#00c853] shrink-0" />
        </button>
      </div>
    );
  }
  ...
  if (isNavigating || navMode === "live") {
    return (
      <div className="absolute top-4 left-4 right-4 z-20 space-y-2">
        {/* Destination banner */}
        <div className="bg-[#0f1e35]/95 backdrop-blur-sm rounded-2xl shadow-xl px-4 py-3 flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-[#00c853] flex items-center justify-center shrink-0">
            <Navigation className="w-4 h-4 text-white" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs text-[#8899aa]">Navigating to</p>
            <p className="text-sm font-bold text-white truncate">{selectedDest?.label}</p>
          </div>
          <button
            onClick={stopNavigation}
            className="w-8 h-8 rounded-xl bg-[#e53935]/20 flex items-center justify-center text-[#e53935] hover:bg-[#e53935]/30 transition-colors"
          >
        ...
        {/* Progress bar */}
        {navMode === "simulated" && (
          <div className="bg-white/97 backdrop-blur-sm rounded-2xl shadow-lg px-4 py-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <Footprints className="w-3.5 h-3.5 text-[#00c853]" />
                <span className="text-xs font-semibold text-gray-700">Simulated Walk</span>
              </div>
              <div className="flex items-center gap-1">
                <Clock className="w-3 h-3 text-gray-400" />
                <span className="text-xs text-gray-500">{remainingMin} min left</span>
              </div>
            </div>
            ...
            <div className="flex justify-between mt-1">
              ...
              <span className="text-[10px] text-gray-400">
                {routeDistanceM}m · {Math.ceil(routeDurationSec / 60)} min walk
              </span>
            </div>
          </div>
        )}
        ...
        {/* Landmark callout */}
        {landmark && (
          <div className="bg-[#00c853] rounded-2xl shadow-lg px-4 py-2.5 flex items-center gap-2">
            <MapPin className="w-3.5 h-3.5 text-white shrink-0" />
            <span className="text-sm font-semibold text-white">{landmark}</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="absolute top-0 left-0 right-0 z-20 bg-white rounded-b-3xl shadow-2xl"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 pt-4 pb-3 border-b border-gray-100">
          ...
          <h2 className="text-base font-bold text-gray-900 flex-1">Get Directions</h2>
          {navMode !== "idle" && (
            <button onClick={stopNavigation} className="text-xs text-[#e53935] font-medium">
              Cancel
            </button>
          )}
        </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={cn(
                ...
                hasGps
                  ? "bg-[#e8faf0] border-[#00c853] text-[#00c853]"
          ...
          {!hasGps && navMode !== "simulated" && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5">
              ...
              <p className="text-xs text-amber-700">
                Live GPS is not available. Use <strong>Simulated Walk</strong> to navigate from a known campus hub.
              </p>
            </div>
          )}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      selectedHub?.id === hub.id
                        ? "bg-[#e8faf0] border-[#00c853]"
                        : "bg-white border-gray-100 hover:border-gray-200"
                    )}
                  >
                    <div className="w-9 h-9 rounded-xl bg-[#e8faf0] flex items-center justify-center shrink-0">
                      <Building2 className="w-4 h-4 text-[#00c853]" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-800 leading-tight">{hub.name}</p>
                      <p className="text-xs text-gray-400 mt-0.5">{hub.description}</p>
                    </div>
            ...
            <div>
              {navMode === "simulated" && (
                <div className="flex items-center gap-2 mb-3">
                  <button
                    ...
                    className="text-xs text-[#00c853] font-medium flex items-center gap-1"
                  >
                    <ChevronDown className="w-3.5 h-3.5 rotate-90" />
                    {selectedHub?.shortName}
                  </button>
                  <ArrowRight className="w-3 h-3 text-gray-300" />
                  <span className="text-xs text-gray-500">Choose destination</span>
                </div>
              )}
              ...
              {selectedDest && (
                <button
                  ...
                  className="mt-3 w-full h-12 rounded-2xl bg-[#00c853] hover:bg-[#00b84a] text-white font-semibold text-sm flex items-center justify-center gap-2 shadow-lg shadow-[#00c853]/20 transition-colors"
                >
